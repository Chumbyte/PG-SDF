...
from utils import plot_distance_map
...
def get2D_dataset(*args, shape_type='circle'):
    ...
    if shape_type == 'circle':
        ...
        # out_shape = Polygon(*args, vertices=[[0.+0.01, 0.+0.01], [0.5+0.01, 0.+0.01], [0.5+0.01, -0.5+0.01],
        #                          [-0.5+0.01, -0.5+0.01], [-0.5+0.01, 0.5+0.01], [0+0.01, 0.5+0.01]])
    elif shape_type == 'U':
        out_shape = Polygon(*args, vertices=
                            [[0.4, 0.4], [0.3, 0.4], [0.3, 0.5], [0.6, 0.5], 
            [0.6, 0.4], [0.5, 0.4], [0.5, -0.5], [-0.5, -0.5], [-0.5, 0.4], [-0.6, 0.4], 
            [-0.6, 0.5], [-0.3, 0.5], [-0.3, 0.4], [-0.4, 0.4], [-0.4, -0.4], [0.4, -0.4], ]
            )
...
if __name__ == "__main__":
    ...
    #                        dataset.dist_img.flatten(), title_text='', show_ax=False, output_path='./
    # /') # plot sdf, indicator function, and points

    # 
    # .plot_shape_data(dataset.grid_points[:, 0], dataset.grid_points[:, 1], dataset.dist_img.flatten(),
    ...
    # 
    # .plot_paper_teaser_images(dataset.grid_points[:, 0], dataset.grid_points[:, 1] ,dataset.dist_img.flatten(),
    #                              clean_points_gt, grid_normals) # plot images for DiGS paper teaser image

    
    plot_distance_map(dataset.grid_points[:, 0], dataset.grid_points[:, 1], dataset.dist_img.flatten(), title_text='', colorscale='Geyser', show_ax=True)
